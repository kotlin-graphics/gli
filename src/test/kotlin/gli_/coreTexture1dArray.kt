...
class coreTexture1dArray : StringSpec() {
    ...
    init {
        ...
        "alloc" {
            ...
            for (format in Format.FIRST .. Format.LAST)
                sizes.forEach {
                    ...
                    val textureA = Texture1dArray(format, size, glm.levels(size), 1)
                    val textureB = Texture1dArray(format, size, glm.levels(size), 1)
        ...
        "query" {
            ...
            val texture = Texture1dArray(Format.RGBA8_UINT_PACK8, Vec1i(2), layers, levels)

            texture.size shouldBe Vec4ub.size * 3 * layers
        ...
        "tex access" {
            ...
            val texture1dArray = Texture1dArray(Format.RGBA8_UINT_PACK8, Vec1i(2), 2, 1)
            texture1dArray.notEmpty() shouldBe true

            val texture0 = texture1dArray[0]
            val texture1 = texture1dArray[1]

            val size0 = texture0.size
            val size1 = texture1.size

            size0 shouldBe Vec4b.size * 2
            size1 shouldBe Vec4b.size * 2
            ...
            val pointer0 = texture1dArray.data<Vec4b>()[0]
            val pointer1 = texture1dArray.data<Vec4b>()[2]
        ...
        "size" {
            ...
            val tests = arrayOf(
                    Test(Format.RGBA8_UINT_PACK8, Vec1i(4), 32),
                    Test(Format.R8_UINT_PACK8, Vec1i(4), 8))

            tests.forEach {
                val texture1dArray = Texture1dArray(it.format, Vec1i(4), 2, 1)
        ...
        "clear" {
            ...
            val texture = Texture1dArray(Format.RGBA8_UNORM_PACK8, Vec1i(8), 1, 5)
            texture clear black

            val texelA = texture.load<Vec4ub>(Vec1i(0), 0, 0)
            val texelB = texture.load<Vec4ub>(Vec1i(0), 0, 1)
            val texelC = texture.load<Vec4ub>(Vec1i(0), 0, 2)
            ...
            val textureView = Texture1dArray(texture, 0, 0, 1, 1)

            val textureImage = Texture1dArray(Format.RGBA8_UNORM_PACK8, Vec1i(4), 1, 1)
    ...
    inline fun <reified T> run(format: Format, testSamples: Array<T>) {
        ...
        val textureA = Texture1dArray(format, dimensions, 3)
        textureA.clear()
        testSamples.forEachIndexed { i, test ->
            textureA.data<T>(2, 0, 1)[i] = test
        }

        val textureB = Texture1dArray(format, dimensions, 3)
        textureB.clear()
        testSamples.forEachIndexed { i, test ->
            textureB.store(T::class, texelCoord[i], 2, 1, test)
        }

        val loadedSamplesA = Array(8) { textureA.load<T>(T::class, texelCoord[it], 2, 1) }

        val loadedSamplesB = Array(8) { textureB.load<T>(T::class, texelCoord[it], 2, 1) }
        ...
        val textureC = Texture1dArray(textureA, 2, 2, 1, 1)
        val textureD = Texture1dArray(textureB, 2, 2, 1, 1)
