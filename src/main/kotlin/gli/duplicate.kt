...
fun duplicate(texture: Texture1d, baseLevel: Int, maxLevel: Int): Texture1d {
    ...
    nmemcpy(memAddress(duplicate.data()), texture.data(0, 0, baseLevel), duplicate.size().L)
...
private fun duplicateImages(
    ...
    for (layerIndex in 0..maxLayer - baseLayer)
        for (faceIndex in 0..maxFace - baseFace) {
            val d = dst.data(layerIndex, faceIndex, baseLevel)
            val s = src.data(baseLayer + layerIndex, baseFace + faceIndex, baseLevel)
